...
#################################################################
# Function:  Ramanome IRCA Grouping function  
# Last update: 2021-09-22, He Yuehui; Shi Huang
#################################################################
...
option_list <- list(
	...
	make_option(c("-o", "--out_dir"), type="character", default='grouping', help="outpath file name[default %default]")
)
...
GlobalBandsAnnfile<- sprintf('%s/databases/Global_bands_annotation.txt',"/home/gene/jinggc/RamEX")
#GlobalBandsAnnfile<- 'Global_bands_annotation.txt'
cor_cutoff<-opts$cor_cutoff
outpath <- opts$out_dir#"outputpath" 
category<-c("group_A", "group_B")
...
global_bands_ann<-read.table(GlobalBandsAnnfile,header=T, sep="\t")
