...
{
  meta_data <- c()
  filefolder <- list.files(foldernames)
  workingpath <- paste(foldernames,"/",filefolder[1],sep = "")
  filenames <- list.files(workingpath, pattern = "*.txt")
  final_data <- spectrum <- as.data.frame(fread(paste(foldernames,"/",filefolder[1],"/",filenames[1],sep=""), header = F, sep = "\t")[,1])
  for (foldername in filefolder)
  {
    ...
    {
      data <- as.data.frame(fread(paste(foldernames,"/",foldername,"/",filename,sep=""),header = F, sep = "\t"))
