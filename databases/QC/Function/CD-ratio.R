...
{
  meta_data <- dplyr::select(as.data.frame(hyperspec),-spc,-.row)
  ...
  data_baseline_normalization_frame_CD_ratio <- dplyr::mutate(data_baseline_normalization_frame,
  ...
  CD_ratio_groups <- dplyr::select(data_baseline_normalization_frame_CD_ratio,CD_ratio)  #需要设置组别，在实验中哪几个参数作为主要变量，需要在这里更换
