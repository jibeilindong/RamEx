...
int printhelp(){
    ...
    cout << "\tRamEX PCA: " << endl;
    cout << "Usage:" << endl;
    cout << "RamEX-IRCA-Generate-Correlation-Matrix [Option]" << endl;
    cout << "Options: " << endl;
...
int main(int argc, char * argv[]){
    ...
    sprintf(command, "Rscript %s/IRCA_gcm.R  -i %s -m %s -c %s -g %s -l %s -p %s -n %s -o %s", R_path.c_str(), Infilename.c_str(), Metafilename.c_str(), Cor_cutoff.c_str(), GlobalBandsAnnfile.c_str(), LocalBandsAnnfile.c_str(), Pos_Edge.c_str(), Neg_Edge.c_str(), Out_path.c_str());
    Run_With_Error(command, Error_file.c_str());
    cout << endl << "IRCA Generate Correlation Matrix Finished"<< endl;
